...
test('checkout asks for customer details before KHQR or Telegram payment', () => {
  ...
  assert.match(clientSource, /customerName: normalizedCustomerName/)
  assert.match(clientSource, /customerPhone: normalizedCustomerPhone/)
  assert.match(clientSource, /phoneHasTelegram: phoneHasTelegram === 'yes'/)
})

test('Bakong paid checkout is stored for admin orders', () => {
  const khqrRoute = readFileSync('app/api/bakong/khqr/route.ts', 'utf8')
  const statusRoute = readFileSync('app/api/bakong/status/route.ts', 'utf8')
  const orderRepository = readFileSync('lib/checkout-orders.ts', 'utf8')
  const schemaSource = readFileSync('db/schema.ts', 'utf8')
  const adminOrdersPage = readFileSync('app/admin/(protected)/orders/page.tsx', 'utf8')

  assert.match(schemaSource, /checkoutOrders/)
  assert.match(orderRepository, /ensureCheckoutOrdersTable/)
  assert.match(khqrRoute, /createPendingCheckoutOrder/)
  assert.match(khqrRoute, /getCheckoutProductSnapshot/)
  assert.match(statusRoute, /markCheckoutOrderPaid/)
  assert.match(statusRoute, /orderStored/)
  assert.match(orderRepository, /paymentStatus: 'paid'/)
  assert.match(adminOrdersPage, /getAdminCheckoutOrders/)
  assert.match(adminOrdersPage, /KHQR checkout requests/)
  assert.doesNotMatch(orderRepository, /BAKONG_API_TOKEN/)
  assert.doesNotMatch(adminOrdersPage, /BAKONG_API_TOKEN/)
})
