    ...
    <div className="min-h-screen">
      ...
      <section className="py-16 sm:py-24 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-sm font-medium text-[var(--color-primary)] mb-4 animate-fade-up">
            {t(`Authorized Hikvision Dealer • ${totalProducts.toLocaleString()} Products`,`ភ្នាក់ងារ Hikvision ផ្លូវការ • ផលិតផល ${totalProducts.toLocaleString()}`)}
          </p>
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap animate-fade-up delay-3">
            ...
            <a href="https://t.me/SANGHAMEUK" target="_blank" rel="noopener noreferrer" className="btn-secondary px-7 py-3">{t('Contact Sales','ទាក់ទងផ្នែកលក់')}</a>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-[var(--color-surface)] border-y border-gray-100">
        <div className="container-page grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[{n:'5+',l:t('Years','ឆ្នាំ')},{n:'100+',l:t('Clients','អតិថិជន')},{n:totalProducts.toLocaleString(),l:t('Products','ផលិតផល')},{n:'24/7',l:t('Support','គាំទ្រ')}].map((s,i) => (
            <div key={i}><div className="text-2xl font-bold text-[var(--color-primary)]">{s.n}</div><div className="text-xs text-gray-500 mt-1">{s.l}</div></div>
          ))}
        </div>
      </section>
