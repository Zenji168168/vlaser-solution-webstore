      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-6">
          <div className="relative flex-grow">
            ...
            <input type="text" placeholder="Search products, SKU, brand..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
              className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-900/40 focus:border-red-900/40 placeholder:text-muted-foreground/50" />
          </div>
          <select value={category} onChange={e => { setCategory(e.target.value); setPage(1) }} className="px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-900/40 min-w-[140px]">
            {CATEGORIES.map(c => <option key={c} value={c}>{c === 'All' ? 'All Categories' : c}</option>)}
          </select>
          <select value={brand} onChange={e => { setBrand(e.target.value); setPage(1) }} className="px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-900/40 min-w-[130px]">
            {BRANDS.map(b => <option key={b} value={b}>{b === 'All' ? 'All Brands' : b}</option>)}
          </select>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-900/40 min-w-[130px]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {paginated.map(p => (
            <Link key={p.id} href={`/products/${p.id}`}>
              <div className="product-card bg-gray-50 border border-gray-100 rounded-2xl overflow-hidden h-full flex flex-col group">
                ...
                <div className="p-3 sm:p-4 flex-grow flex flex-col">
                  <span className="text-[9px] font-semibold text-primary/70 uppercase tracking-wider">{p.brand}</span>
                  <h3 className="font-medium text-xs sm:text-sm mt-0.5 line-clamp-2 leading-snug group-hover:text-primary transition-colors">{p.name}</h3>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-10">
            <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1} className="px-3 py-2 text-xs font-medium border border-border rounded-lg hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">← Prev</button>
            <div className="flex items-center gap-1">
              {Array.from({length: Math.min(totalPages, 7)}, (_, i) => {
                ...
                return (
                  <button key={p} onClick={() => setPage(p)} className={`w-8 h-8 text-xs font-medium rounded-lg transition-colors ${page === p ? 'bg-primary text-white' : 'hover:bg-gray-50 border border-gray-100'}`}>{p}</button>
                )
              })}
            </div>
            <button onClick={() => setPage(p => Math.min(totalPages, p+1))} disabled={page===totalPages} className="px-3 py-2 text-xs font-medium border border-border rounded-lg hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">Next →</button>
