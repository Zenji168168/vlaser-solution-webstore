    ...
    <article className="group h-full min-w-0">
      <Link href={`/products/${product.id}`} className="card flex h-full min-w-0 flex-col focus-ring" aria-label={`${t('View details for', 'មើលព័ត៌មាន')} ${product.name}`}>
        ...
        <div className="flex min-w-0 flex-1 flex-col p-3 sm:p-4">
          ...
          <h3 className="mt-2 min-h-[3.8rem] text-sm font-bold leading-snug text-gray-950 line-clamp-3 transition-colors duration-200 group-hover:text-[var(--color-primary)] [overflow-wrap:anywhere]">
            {product.name}
          </h3>
          ...
          <div className="mt-auto pt-4">
            <div className="flex items-end justify-between gap-2">
              ...
              <span className="hidden text-[11px] font-bold text-gray-600 sm:inline">{t('Details', 'លម្អិត')}</span>
            </div>
            <div className="mt-3">
              <span className="inline-flex min-h-10 w-full min-w-0 items-center justify-center rounded-xl bg-gray-950 px-2 text-center text-xs font-bold text-white transition-colors duration-200 group-hover:bg-[var(--color-primary)] sm:px-3">
